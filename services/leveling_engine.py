...
def _count_duplicated_slots(franja_counts):
...
def _can_add_franja(tech,franja,tf,to,now_h):
    cur=tf.get(tech,{}).get(franja,0)
    if cur>=MAX_ORDERS_PER_SLOT: return False
    if cur>=1 and _count_duplicated_slots(tf.get(tech,{}))>=MAX_DUPLICATED_SLOTS: return False
    fs,_=parse_franja_hours(franja)
    if fs and fs>=14.5:
        tarde=sum(c for f,c in tf.get(tech,{}).items() if (parse_franja_hours(f)[0] or 0)>=14.5)
        if tarde>=2: return False
    return True
...
def _alerts(orders,idx,now_dt):
    ...
    for o in orders:
        if norm_status(o['estado'])=='en sitio':
            upd=_parse_updated_at(o.get('updated_at',''))
            if upd:
        ...
        for franja,count in fm.items():
            if count>=OVERLOAD_PER_SLOT:
                alerts.append({'tipo':'SOBRECARGA_FRANJA','severidad':'alta' if count>OVERLOAD_PER_SLOT else 'media',
                    'tecnico':tech,'franja':franja,'count':count,'detalle':f"{tech} tiene {count} ordenes en {franja}"})
    for tech,total in idx['tech_total'].items():
        if tech=='SIN_ASIGNAR': continue
        if total>MAX_ABSOLUTE_LOAD:
            alerts.append({'tipo':'SOBRECARGA_TOTAL','severidad':'alta','tecnico':tech,'total':total,'detalle':f"{tech} tiene {total} ordenes (max:{MAX_ABSOLUTE_LOAD})"})
        elif total>MAX_IDEAL_LOAD:
            alerts.append({'tipo':'SOBRECARGA_TOTAL','severidad':'media','tecnico':tech,'total':total,'detalle':f"{tech} tiene {total} ordenes (techo:{MAX_IDEAL_LOAD})"})
    for tech,total in idx['tech_total'].items():
        if tech=='SIN_ASIGNAR': continue
        pending=idx['tech_pending'].get(tech,0)
        if total<MIN_IDEAL_LOAD and pending>0:
            alerts.append({'tipo':'CARGA_BAJA','severidad':'media','tecnico':tech,'total':total,'detalle':f"{tech} tiene solo {total} ordenes (min:{MIN_IDEAL_LOAD})"})
    for o in orders:
        if o['movible']:
...
def _score(order,donor,receiver,idx,now_h):
    ...
    if not _can_add_franja(receiver,order['franja'],tf,to,now_h): return -9999.0
    score+=300
...
def _suggestions(orders,idx,now_h):
    ...
    for order in movables:
        ...
        if donor=='SIN_ASIGNAR': motivo=f"Orden sin tecnico asignar a {best_r} ({rv} ords)"
        ...
        else: motivo=f"Balance: {donor}({dv})->{best_r}({rv}) ords"
        if sob: motivo+=f" ⚠ {best_r} quedara con {rv+1} ords"
        ben=[]
...
def run_leveling(raw_orders):
    ...
    sugs=_suggestions(orders,idx,now_h)
    cpt=[]
    ...
    def en(o): return {k:o[k] for k in ['id','tecnico','estado','estado_clase','franja','tipo','zona','subzona','direccion','movible','progress','updated_at','lat','lon']}
    al_crit=sum(1 for a in alerts if a.get('severidad') in ('critica','alta'))
    ts=len([t for t in idx['tech_orders'] if t!='SIN_ASIGNAR'])
    ...
    return {'generado_en':now_dt.strftime('%Y-%m-%d %H:%M:%S'),
        'resumen':{'total_ordenes':len(orders),'movibles':len(movibles),'bloqueadas':len(bloqueadas),
            ...
            'tecnicos_total':ts,'tecnicos_sobrecargados':tsob,'tecnicos_con_capacidad':tcap,
